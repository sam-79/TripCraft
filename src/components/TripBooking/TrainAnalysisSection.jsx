import { Card, Row, Col, Table, Statistic, Typography, Space, Tag, Empty, List, Divider, Button, Tooltip } from "antd";
import { motion } from "framer-motion";
import { ClockCircleOutlined, ThunderboltOutlined, DollarCircleOutlined, StarOutlined, PlusOutlined, MinusCircleOutlined } from "@ant-design/icons";
...
const getTrainAvailabilityTag = (status) => {
    ...
    if (status.includes("WL") || status.includes("RLWL")) return <Tag color="orange">{status}</Tag>;
    return <Tag color="red">{status}</Tag>;
};

// --- Train Analysis Section ---
const TrainAnalysisSection = ({ analysis, bookingList, onAddToBooking }) => {
    const legAnalysis = analysis?.leg_wise_analysis?.[0];
    if (!legAnalysis || !analysis.overall_statistics) {
        return <Empty description="Detailed train analysis not available for this leg." />;
    }

    // --- Action Column for adding/removing from booking list ---
    const actionColumn = {
        title: "Action",
        key: "action",
        align: "center",
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                <div>
                    <Text strong>{record.train_name}</Text><br />
    ...
    const flattenedTrainsData = legAnalysis.available_trains?.flatMap(train =>
    ...
    // --- Fare Analysis Table ---
    const fareByClass = Object.entries(legAnalysis.fare_analysis?.by_class || {}).map(([cls, stats]) => ({
        key: cls,
        class: cls,
        ...stats,
    }));
    ...
    const fareColumns = [
        ...
        { title: "Avg Fare", dataIndex: "average", key: "average", render: formatCurrency },
        { title: "Min Fare", dataIndex: "min", key: "min", render: formatCurrency },
        { title: "Max Fare", dataIndex: "max", key: "max", render: formatCurrency },
        { title: "Median", dataIndex: "median", key: "median", render: formatCurrency },
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} md={6}>
                    <Card bordered={false}>
                        <Statistic title="Total Trains Found" value={analysis.overall_statistics.total_trains_found} />
                    </Card>
                </Col>
                <Col xs={24} sm={12} md={6}>
                    <Card bordered={false}>
                        <Statistic title="Available Tickets (Est.)" value={legAnalysis.summary.total_available_seats} suffix="Seats" />
                    </Card>
                </Col>
                <Col xs={24} sm={12} md={6}>
                    <Card bordered={false}>
                        <Statistic title="Fastest Train Duration" value={fastestTrains?.[0]?.duration || "N/A"} prefix={<ClockCircleOutlined />} />
                    </Card>
                </Col>
                <Col xs={24} sm={12} md={6}>
                    <Card bordered={false}>
                        <Statistic title="Avg Fare (Sleeper)" value={formatCurrency(legAnalysis.fare_analysis?.by_class?.Sleeper?.average)} />
                    </Card>
                </Col>
            </Row>
            ...
            <Title level={4}><DollarCircleOutlined /> Fare Analysis by Class</Title>
            <Table
                columns={fareColumns}
            ...
            <Title level={4}><ThunderboltOutlined /> Fastest Trains</Title>
            <List
                bordered
                ...
                renderItem={(train) => (
                    <List.Item>
                        <Space direction="vertical">
                            <Text strong>{train.train_name} ({train.train_number})</Text>
                            <Text>Duration: {train.duration}</Text>
                            <Text>Departure: {train.departure_time} → Arrival: {train.arrival_time}</Text>
                        </Space>
                    </List.Item>
                )}
            />
            ...
            {/* --- Cheapest Options --- */}
            <Title level={4}><DollarCircleOutlined /> Cheapest Options</Title>
            {Object.entries(cheapestOptions).map(([classType, trains]) => (
                <Card key={classType} title={classType.replace(/_/g, " ").toUpperCase()} size="small" style={{ marginBottom: 16 }}>
                    <List
                        size="small"
                        dataSource={trains}
                        renderItem={(t) => (
                            <List.Item>
                                <Space wrap>
                                    <Text strong>{t.train_name} ({t.train_number})</Text>
                                    <Tag>{t.class}</Tag>
                                    <Text>{formatCurrency(t.fare)}</Text>
                                    {getTrainAvailabilityTag(t.availability)}
                                    <Text type="secondary">({t.departure_time}, {t.duration})</Text>
                                </Space>
                            </List.Item>
                        )}
                    />
            ...
            {/* --- Recommendations --- */}
            <Title level={4}><StarOutlined /> Recommendations</Title>
            <Row gutter={[16, 16]}>
